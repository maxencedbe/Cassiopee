    ...
    for idx, row in df.iterrows():
        ...
        while i < len(df.columns):
            ...
            if isinstance(cell, str) and cell.upper() in candidats_cibles:
                pct_col_index = i + 3  # 3 colonnes à droite pour le %
...
def filtre_et_ajoute(df, top10, noms_communes, description_col):
    ...
    # Filtrer df pour garder uniquement les lignes dans communes top10
    filtered_rows = []
    # Dictionnaire pour récupérer la valeur % par commune
    ...
    for idx, row in df.iterrows():
        ...
        if (code_dep, code_com) in communes_set:
            # Ajouter la colonne % inference à cette ligne
